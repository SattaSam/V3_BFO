(function (global) {
  ...
  // Politique canonique de peuplement : aucune couche parallèle ne peut
  // désormais remplacer ces règles selon l'ordre de chargement.
  const base=BF.BiomeRules;
  if(!base?.getMapPopulation||BF.BiomePopulationPolicy?.version==="population-r3.3")return;
  ...
  const patch=(def,pop)=>{
   ...
   const fungalMushroomMap=profile==="fungal"||biomeId==="fungal"||/fong|fung|champignon|mushroom|spore/.test(context);
   const swampMushroomMap=profile==="swamp"||/marais|swamp/.test(context);
   const giantMushroomMap=fungalMushroomMap||swampMushroomMap;
   const floating=/flott|suspend|levitat/.test(context);
   const mapNumber=Number(def?.number);
   const discoveryIndex=Number(def?.generator?.discoveryIndex);
   const tutorial=!!(
   ...
   dec=dec.filter(([t])=>!CAMP_ONLY.has(t)&&!/^(?:camp|base|reinforced_base|base_fire|wood_plane|toile)$/.test(t));
   if(profile==="desert"){
   ...
   if(fungalMushroomMap){addOrRaise(dec,"lantern_mushrooms",24);addOrRaise(dec,"giant_mushroom",12);rocks=Math.min(rocks,5);}
   else if(swampMushroomMap)addOrRaise(dec,"giant_mushroom",7);
   ...
   if(!giantMushroomMap)dec=dec.filter(([t])=>!GIANT_MUSHROOMS.has(t));
