...
const VERSION="1.1.5",KEY="bluefox_music_settings_v1",clamp=v=>Math.max(0,Math.min(1,Number(v)||0)),clock=()=>global.performance?.now?.()||Date.now();
...
class AdaptiveMusicEngine{
 constructor(options={}){
  ...
  this.timer=null;this.fadeTimer=null;this.retryTimer=null;this.retryCount=0;this.transitioningAudio=false;this.playGeneration=0;this.segmentStartedAt=0;this.lastTransition=null;this.lastError=null;this.settings=this.loadSettings();
 ...
 setMusicalState(detail={}){
  ...
  this.signal=next;if(!changed)return false;this.lastSignalChange=Date.now();if(detail.defer)return true;
  const desired=this.chooseSequence();if(!desired||desired===this.sequenceId||!this.started)return true;
  this.pending={id:this.context,priority:this.priority,sequenceId:desired};return true;
 }
 ...
 applyPending(){
  ...
  this.context=p.id;this.priority=Number(p.priority??this.priority);this.selectSequence(true);
  const fade=p.sequenceId?3:Number(cat.contextProfiles[p.id].crossfadeSec??cat.transitions.standardCrossfadeSec);
 ...
 stop(){this.started=false;this.playGeneration++;this.transitioningAudio=false;this.clearTimer();global.clearTimeout(this.retryTimer);this.retryTimer=null;if(this.fadeTimer)global.clearInterval(this.fadeTimer);this.fadeTimer=null;this.decks.forEach(d=>{d.token++;d.audio.pause();d.audio.volume=0;});}
