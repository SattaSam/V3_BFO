...
function ensureAutonomyControl(settings){
  let sectionTitle=[...settings.querySelectorAll("h3,h4,strong")].find(el=>norm(el.textContent)==="priorites");
  if(!sectionTitle) return;
  let row=sectionTitle.closest(".settings-section-title,.settings-heading,.section-title")||sectionTitle.parentElement;
  if(!row) return;
  row.classList.add("bac-priority-heading-row");
  let trigger=row.querySelector(".bac-autonomy-trigger");
  if(!trigger){
    trigger=document.createElement("button");
    trigger.type="button";
    trigger.className="bac-autonomy-trigger";
    trigger.textContent="Autonomie";
    trigger.setAttribute("aria-haspopup","dialog");
    row.appendChild(trigger);
  }
  if(!document.querySelector(".bac-autonomy-modal")){
    const modal=document.createElement("div");
    modal.className="bac-autonomy-modal";
    modal.hidden=true;
    modal.innerHTML=`<div class="bac-autonomy-dialog" role="dialog" aria-modal="true" aria-label="Réglage de l’autonomie de BlueFox">
      <button type="button" class="bac-autonomy-close" aria-label="Fermer">×</button>
      <h3>Autonomie de BlueFox</h3>
    ...
    modal.querySelector(".bac-autonomy-close").addEventListener("click",close);
    modal.addEventListener("click",event=>{if(event.target===modal)close();});
    modal.querySelectorAll(".bac-autonomy-choice").forEach(button=>{
      button.addEventListener("click",()=>setAutonomyMode(button.dataset.mode));
    });
  }
  const modal=document.querySelector(".bac-autonomy-modal");
  if(trigger.dataset.connected!=="1"){
    trigger.dataset.connected="1";
