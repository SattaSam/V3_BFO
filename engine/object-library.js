(function (global) {
  ...
  const productionObject = (THREE, palette, variant, type) => {
    ...
    if (type === "strong_rock" || type === "large_rock") {
      ...
      colliders = [{ offset: new THREE.Vector3(), radius: 0.72 }];
    } else if (type === "fauna_straw_ball") {
      const straw = material(THREE, { color: 0x76644f, roughness: 1, metalness: 0 });
      const darkStraw = material(THREE, { color: 0x5e4a38, roughness: 1, metalness: 0 });
      const ball = new THREE.Mesh(new THREE.IcosahedronGeometry(0.46, 2), straw);
      ball.name = "FaunaStrawBallBody";
      ball.position.y = 0.46;
      ball.scale.set(1.03, 0.98, 1);
      root.add(ball);
      [[0, 0, 0], [Math.PI / 2, 0, 0], [0, Math.PI / 2, 0]].forEach((rotation, index) => {
        const strand = new THREE.Mesh(
          new THREE.TorusGeometry(0.45 - index * 0.012, 0.018, 5, 24),
          index === 1 ? darkStraw : straw
        );
        strand.name = "FaunaStrawBallStrand";
        strand.position.y = 0.46;
        strand.rotation.set(...rotation);
        root.add(strand);
      });
      root.userData.faunaManipulable = true;
      root.userData.faunaToolUseMission = "FAU-10";
      hitbox = makeHitbox(THREE, root, 0.55, 0.95, type);
    } else if (type === "bush") {
  ...
  const PRODUCTION_SPECS = Object.freeze([
    ...
    { id: "DOC-RES-WOOD-M-001", type: "tree_fallen", label: "Bois", category: "resources", subtype: "fallen_alien_branch", size: "M", rarity: "common", biomes: ["forest", "jungle", "swamp", "plain", "alien"], scenes: ["Bois mort", "Camp de construction"], states: ["au sol", "repéré", "inspecté", "collecté", "transformé"], actions: ["observe", "inspect", "collect", "transform"], defaultAction: "collect", collectable: true, inspectable: true, obstacle: true, respawn: 210, inventoryKey: "wood", quantity: 1, exploitability: "combustible/transformable", family: "flora", resourceFamily: "wood", research: ["botany", "materials", "engineering"], tags: ["resource", "wood", "construction", "obstacle"], spawnCost: 3, maxPerZone: 6, minDistance: 2.2, maxSlope: 24, radius: 1.2, volume: "medium", curiosity: 0.45, harvest: 0.72, missions: ["camp", "construction refuge", "fabrication de planches"], note: "Bois tombé collectable : 1 unité de bois.", decision: "validated" },
  ...
  const PRODUCTION_OBJECT_LIBRARY = Object.freeze(Object.fromEntries(PRODUCTION_SPECS.map((spec) => [
    ...
    Object.freeze({
      ...
      resource: spec.collectable ? { family: spec.resourceFamily, exploitability: spec.exploitability, inventoryKey: spec.inventoryKey, quantity: Math.max(1, Number(spec.quantity) || 1) } : null,
